...
class MarriageModel:
    ...
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    def random_sd(self, number_of_matchings=1, **kwargs):
        
        proposers = self.proposers
        receivers = self.receivers
        preferences = [proposers, receivers]
        mu = {}
        new_proposer = list(proposers)
        
        random.shuffle(new_proposer)
        priority_lst = new_proposer
        
        for rnd in range(len(priority_lst)):
            ind = priority_lst[rnd]
            mu[ind] = preferences[0][ind][0]
            
            del preferences[0][ind]
            
            for lst in preferences[0].values():
                if mu[ind] in lst:
                    lst.remove(mu[ind])
    ...
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    def random_ttc(self, number_of_matchings=1, **kwargs):
        
        proposers = self.proposers
        receivers = self.receivers
        preferences = [proposers, receivers]
        mu = {person: None for person in list(preferences[0])}
        new_proposer = list(proposers)
        new_receiver = list(receivers)
        
        shuffle_receiver = list(itertools.permutations(new_receiver))
        random.shuffle(shuffle_receiver)
        permutation_receive = shuffle_receiver        

        for j in range(len(new_proposer)):
            mu[new_proposer[j]] = permutation_receive[0][j]
        
        mu_final = {}
        rnd = 0
        for k in range(len(new_receiver)):
            rnd += 1
            cycle = {}
            ind_now = new_proposer[0]
            
            for w in range(len(new_proposer)):
                cycle[ind_now] = preferences[0][ind_now][0]
                for key, value in mu.items():
                    if value == preferences[0][ind_now][0]:
                        ind_now = key
                if ind_now in cycle:
                    cycle_list = list(cycle.items())
                    initial = list(cycle.keys()).index(ind_now)
                    cycle_list = cycle_list[initial:]
                    cycle = dict(cycle_list)
                    break
       
            mu_final.update(cycle)
            
            for p_key, p_value in cycle.items():
                for lst in preferences[0].values():
                    if p_value in lst:
                        lst.remove(p_value)

                new_proposer.remove(p_key)
                del mu[p_key]
                del preferences[0][p_key]
            
            if len(mu) == 0:
                print(rnd)
                return(dict(sorted(mu_final.items())), rnd)
                break
    ...
    def enumeration_stability(self, number_of_matchings=1, **kwargs):

        preferences = [self.proposers, self.receivers]
        mu = {person: None for person in list(preferences[0])}
        new_proposer = list(self.proposers)
        new_receiver = list(self.receivers)
        ...
        flag = 0
        flag_DA = 0
        for i in range(len(permutation_receive)):
            for j in range(len(new_proposer)):
                mu[new_proposer[j]] = permutation_receive[i][j]
                
            blocking_pair = self.is_stable(mu, preferences)
            if blocking_pair is True:
                flag +=1
                print('The algorithm ran {} rounds to reach the following stable matching:'.format(i+1))
                print(mu)
                if mu == self.Deferred_Acceptance():
                    flag_DA = i+1
                if flag == number_of_matchings:
                    break
            else:
                pass
